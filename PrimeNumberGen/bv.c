...
//NOTE: BitVector ADT code used from asgn 4
...
BitVector *bv_create(uint32_t bit_len) {
  ...
  if (bv != NULL) {           // Check if object is null
    bv->length = bit_len + 1; // Set length of array to lenght value passed in.
    bit_len = (bit_len / 8) +
              1; // compute bytes to allocate given the amount of bits needed.
    bv->vector = (uint8_t *)calloc(
        bit_len + 1, sizeof(uint8_t)); // Allocate memory of the size of bytes
